...
====================================================
Covers shape invariants, edge cases, determinism, and mathematical
sanity for the quant-core modules.
...
class TestParameterEstimation:
    """Tests for models.parameter_estimation.estimate_parameters."""
    ...
    def test_raises_on_single_observation(self) -> None:
        """A single observation is insufficient for σ (ddof=1)."""
    ...
    def test_known_values(self) -> None:
        """Verify μ and σ against hand-computed values."""
...
class TestGBMSimulation:
    """Tests for simulation.gbm.simulate_gbm."""
    ...
    def test_output_shape(self, horizon: int, num_paths: int) -> None:
        """Output must always be (horizon, num_paths)."""
        paths = simulate_gbm(
    ...
    def test_all_prices_positive(self) -> None:
        """GBM prices are strictly positive by construction (exp > 0)."""
        paths = simulate_gbm(
    ...
    def test_different_seeds_differ(self) -> None:
        """Different seeds must (almost certainly) produce different paths."""
        kwargs = dict(S0=100.0, mu=0.001, sigma=0.02, horizon=10, num_paths=50)
...
class TestSimulationEngine:
    """Tests for simulation.engine.run_simulation."""

    def test_output_shape(self) -> None:
        """Engine must return 1-D array of shape (num_paths,)."""
    ...
    def test_deterministic_with_seed(self) -> None:
        """Same seed → same terminal returns."""
        returns = pd.Series(np.random.default_rng(0).normal(0.0005, 0.02, 252))
